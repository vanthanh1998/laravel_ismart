...
class SliderController extends Controller
{
    ...
    public function post_edit(Request $request,$id){
    	...
    	$img_current = 'upload/slider/'.$request->img_current;// tạo input hidden có name là img_current~~~ nhu7 là thư mục chưa image
    ...
    public function get_delete($id){
    	$slider = Slider::find($id);
        File::delete('upload/slider/'.$slider['image']);
    	$slider->delete($id);
    	return response()->json([
    	    'success' => true,
            'message' =>'Thành công',
    ...
    public function get_delete_all(Request $request){
        ...
        foreach($slider as $image){
            File::delete('upload/slider/'.$image['image']);// xóa hình trong list_image
        }
        $slider->delete();
        return response()->json([
            'success'=>true,
