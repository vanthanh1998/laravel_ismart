...
class HomeController extends Controller
{
	...
	public function index(Request $request){
        return view('pages.home',compact('d','strDate1'));
    }
    ...
    public function ctsp($id,Request $request){
    	...
    	return view('pages.product.detail_product',compact('ctsp','list_image','loaisp','danhmucsp','product_cate_same','danhgia','arrayRatings'));
    }
    ...
    public function post_lienhe_mobile(Request $request){
        ...
        $messages = [
            'name_contact.required' => 'Name không được để trống',
            'email_contact.required' => 'Email không được để trống',
            'email_contact.email' => 'Email không đúng định dạng',
    ...
    public function resetPassword(Request $request){
        ...
        if(!$checkUser)
        {
            return redirect('dashboard')->with('danger','Xin lỗi ! Đường dẫn lấy lại mật khẩu không đúng, xin vui lòng kiểm tra lại.');
        }
    ...
    public function saveResetPassword(ResetPasswordRequest $requestResetPassword){
        if($requestResetPassword->password) // nếu $requestResetPassword tồn tại
        {
            ...
            return redirect('dang-nhap')->with('success','
                Mật khẩu đã được đổi thành công, mời bạn đăng nhập');
        }
    }
}
